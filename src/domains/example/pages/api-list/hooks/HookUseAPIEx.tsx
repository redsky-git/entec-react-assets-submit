...
interface IHookUseAPIExProps {
	test?: string;
}

const HookUseAPIEx: IComponent<IHookUseAPIExProps> = (): JSX.Element => {
	const { data, fetch, setData } = useAPI('exampleStore/posts');
	//const { data: searchData, fetch: searchFetch } = useAPI('exampleStore/search');
				...
				<div className="mx-auto flex w-full  min-w-0 flex-1 flex-col gap-8 px-4 py-6 text-neutral-800 md:px-0 lg:py-8 dark:text-neutral-300">
					<div className="flex flex-col gap-2">
						...
						<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
							frontend에서 api서버와 통신하기 위해 사용하는 훅 함수입니다.
						</p>
						<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
							api서버와 통신 결과 response 데이터를 가져오고 전역 State에 저장합니다.
						</p>
					</div>
						...
						<Alert>
							...
							<AlertDescription>
								...
								<ul className="list-inside list-disc text-sm">
									...
									<li>
										<strong>REST API</strong> 호출 및 결과 데이터 사용을 위한 <strong>useAPI</strong> 훅을 사용하면
...
HookUseAPIEx.displayName = 'HookUseAPIEx';
export default HookUseAPIEx;
