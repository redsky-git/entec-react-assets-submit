...
interface IRouterGoBackExProps {
	test?: string;
}

const RouterGoBackEx: IComponent<IRouterGoBackExProps> = (): JSX.Element => {
	// 이전화면으로 이동 handler
	const handlerRouterGoBack = () => {
		$router.goBack();
	};
				...
				<div className="mx-auto flex w-full  min-w-0 flex-1 flex-col gap-8 px-4 py-6 text-neutral-800 md:px-0 lg:py-8 dark:text-neutral-300">
					<div className="flex flex-col gap-2">
						...
						<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
							...
							메서드를 통해 화면이 이동 된 후, 다시 이전 화면으로 되돌아가기 위하여 <strong>history back</strong>할 때
							사용하는 메서드입니다.
						</p>
					</div>
					<div className="w-full flex-1 *:data-[slot=alert]:first:mt-0">
						<Alert>
							<AlertCircleIcon />
							<AlertTitle>$router.goBack() 관련</AlertTitle>
							<AlertDescription>
								<ul className="list-inside list-disc text-sm">
									<li>
										<strong>$router.goBack()</strong>함수는 내부적으로{' '}
										<strong>
									...
									<li>
										...
										$router.goBack() 를 사용하기 위하여 따로 <strong>import</strong>를 하지 않아도 됩니다.
									</li>
								</ul>
							</AlertDescription>
						</Alert>
						...
						<div className="flex flex-col gap-2 pt-6">
							...
							<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
								이동된 화면에서 다시 이전화면으로 되돌아가는 예제입니다.
							</p>
						</div>
								...
								<TabsContent value="preview">
									<Card>
										<CardContent className="pt-5 pb-5 flex items-center justify-center">
											<Button onClick={handlerRouterGoBack}>버튼을 누르면 이전화면으로 되돌아갑니다.</Button>
										</CardContent>
									</Card>
								</TabsContent>
...
RouterGoBackEx.displayName = 'RouterGoBackEx';
export default RouterGoBackEx;
