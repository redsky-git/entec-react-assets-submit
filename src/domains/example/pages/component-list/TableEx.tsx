...
interface ITableExProps {
	test?: string;
}

const TableEx: IComponent<ITableExProps> = (): JSX.Element => {
				...
				<div className="mx-auto flex w-full  min-w-0 flex-1 flex-col gap-8 px-4 py-6 text-neutral-800 md:px-0 lg:py-8 dark:text-neutral-300">
					<div className="flex flex-col gap-2">
						<div className="flex items-start justify-between">
							<h1 className="scroll-m-20 text-4xl font-semibold tracking-tight sm:text-3xl xl:text-4xl">Table</h1>
							<div className="docs-nav bg-background/80 border-border/50 fixed inset-x-0 bottom-0 isolate z-50 flex items-center gap-2 border-t px-6 py-4 backdrop-blur-sm sm:static sm:z-0 sm:border-t-0 sm:bg-transparent sm:px-0 sm:pt-1.5 sm:backdrop-blur-none">
								&nbsp;
							</div>
						</div>
						<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
							<strong>Table</strong> 컴포넌트는 기본적인 HTML 테이블 구조를 기반으로 하며, 접근성(Accessibility)과
							커스터마이징에 중점을 둔 재사용 가능한 React 컴포넌트들의 집합입니다.
						</p>
					</div>
					<div className="w-full flex-1 *:data-[slot=alert]:first:mt-0">
						...
						<div className="flex flex-col gap-2 pt-6">
							...
							<ul className="list-disc ml-4">
								<li>
									<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
										<strong>entec-react-assets</strong> 라이브러리에서 제공하는 <strong>Table</strong> 컴포넌트를
										이용하여, 데이터 배열을 프로그래밍으로 세팅해서 보여주는 기본적인 테이블 예제입니다.
									</p>
								</li>
								<li>
									<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
										<strong>Table</strong> 컴포넌트에서 제공하는, 테이블관련 컴포넌트는 모두{' '}
										<strong>
											<code className="bg-muted relative rounded-md px-[0.3rem] py-[0.2rem] font-mono text-[0.9rem] break-words outline-none">
		...
		},
		{
			...
			paymentMethod: 'Credit Card',
						...
						<div className="flex flex-col gap-2 pt-6">
							...
							<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
								<strong>Table</strong> 컴포넌트와 <strong>TailwindCSS</strong>를 활용하여 디자인된 테이블 예제입니다.
							</p>
						</div>
						<div className="w-full flex-1 py-4">
							<Tabs defaultValue="preview">
