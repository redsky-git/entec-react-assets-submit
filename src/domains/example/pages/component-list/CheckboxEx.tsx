...
interface ICheckboxExProps {
	test?: string;
}

const CheckboxEx: IComponent<ICheckboxExProps> = (): JSX.Element => {
	// useEffect hooks
	useEffect(() => {
		// ...
	}, []);
				...
				<div className="mx-auto flex w-full  min-w-0 flex-1 flex-col gap-8 px-4 py-6 text-neutral-800 md:px-0 lg:py-8 dark:text-neutral-300">
					<div className="flex flex-col gap-2">
						<div className="flex items-start justify-between">
							<h1 className="scroll-m-20 text-4xl font-semibold tracking-tight sm:text-3xl xl:text-4xl">Checkbox</h1>
							<div className="docs-nav bg-background/80 border-border/50 fixed inset-x-0 bottom-0 isolate z-50 flex items-center gap-2 border-t px-6 py-4 backdrop-blur-sm sm:static sm:z-0 sm:border-t-0 sm:bg-transparent sm:px-0 sm:pt-1.5 sm:backdrop-blur-none">
								&nbsp;
							</div>
						</div>
						<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
							<strong>Checkbox</strong> 컴포넌트는 여러가지 옵션 중 하나 이상을 선택할 수 있게하는 컴포넌트입니다.
						</p>
					</div>
											...
											<div className="flex items-start gap-3">
												...
												<div className="grid gap-2">
													...
													<p className="text-muted-foreground text-sm">
														이 체크박스를 클릭하면 이용 약관에 동의하는 것으로 간주됩니다.
													</p>
												</div>
											</div>
										</CardContent>
										...
										<CardContent className="pt-5 pb-5 flex items-center justify-center">
											<div className="flex items-center gap-3">
												<Checkbox
													...
													disabled
												/>
												<label htmlFor="toggle">Disabled 처리됨</label>
											</div>
										</CardContent>
									</Card>
								</TabsContent>
		...
		<div className="flex items-center gap-3">
			...
			<label htmlFor="toggle">Disabled 처리됨</label>
						...
						<div className="flex flex-col gap-2 pt-6">
							...
							<p className="text-muted-foreground text-[1.05rem] text-balance sm:text-base">
								<strong>Checkbox</strong> 컴포넌트를
								<code className="bg-muted relative rounded-md px-[0.3rem] py-[0.2rem] font-mono text-[0.8rem] break-words outline-none">
									&lt;label&gt;
								</code>
								태그로 감싸서 전체 UI를 선택할 수 있게 적용한 예제.
							</p>
						</div>
						<div className="w-full flex-1 py-4">
							<Tabs defaultValue="preview">
